...
const PostSeoulItem = ({ idx, postData }) => {
  ...
  useEffect(() => {
    ...
    const fontDataTextArea = document.getElementById(`seoul_textarea_${idx}`);
  ...
  return (
    <div className="fontData" id={`seoul_${idx}`}>
      <link rel="stylesheet" type="text/css" href={postData.webFontPath} />
      <div className="info_box" id={`seoul_color_${idx}`}>
        <div className="deleteTitle">
          <div className="deleteTitle__Title">{postData.title}</div>
          {isowner && (
            <div className="deleteTitle__deleteButton" onClick={handleDelete}>
              삭제
            </div>
          )}
        </div>
        <div className="font_info">
          <div className="font_first_row_box"></div>
          <div className="font_user_info">
            <span style={{ fontSize: "10px" }}>Created By.</span>
            <span style={{ fontWeight: "bold", fontSize: "16px" }}>
              {postData.userLocation}_{postData.userName}_
